         ...
         function getPatients(e){
            ...
            $("#ptSearch").ready(function(){
                  for( inst in curInstance )
                    {
                      $('.dropdown-menu').append('<li role="presentation"><a role="menuitem" tabindex="-1" data-inp="' + 4 + '" data-start="' + curInstance[inst].STARTDATE + '" data-end="' + curInstance[inst].ENDDATE + '" href="#">' + curInstance[inst].INSTANCE + '</a></li>');
                    }
                  $('a').on('click', function(){
    ...
    var localName = this.name;
          ...
          this.chartData = this.chartData.filter(function (d) {return d.ABSTRACTIONTYPE !== "Iso RI pause" && d.ABSTRACTIONTYPE !== "Unclassified"});
           ...
           this.chartData.forEach( function (d) {
            //d.ACTUALSTARTTIME =  d3.time.hour.offset(parseDate3(d.ACTUALSTARTTIME),- 2);
            d.ACTUALSTARTTIME =  d3.time.day.offset(parseDate3(d.ACTUALSTARTTIME), -5);
            d.days = d.ACTUALSTARTTIME;
            d.minutes = d.ACTUALSTARTTIME.getHours();
            //d.ACTUALENDTIME =  parseDate3(d.ACTUALENDTIME);
            d.ACTUALENDTIME =  d3.time.day.offset(parseDate3(d.ACTUALENDTIME), -5);
       ...
       */
        ...
        var maxBins = binNames.length;
          ...
          this.chartData.forEach( function(d) {
                            ...
                            d.DAY = d3.time.day.offset(parseDate(d.DAY), -5);
                            d.values = [];
          ...
          //this.ext = [d3.time.hour.offset(this.ext[0],- 2),this.ext[1]]
              yS.domain(binNames);
              xS.domain(this.ext);
